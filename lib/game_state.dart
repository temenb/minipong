...
class GameState {
  ...
  int get firPlayer => 1;
  int get secPlayer => 2;
  int get greenPlayer => 1;
  int get pinkPlayer => 2;
  ...
  void deleteLogEntry(int index) {
    if (index >= 0 && index < scoreLog.length) {
      ...
      // Пересчитываем счет игроков вручную
      // (player1Score и player2Score пересчитываются через геттеры)
      // Если после удаления нет голов, сбрасываем выбранных игроков
      if (scoreLog.isEmpty) {
        selectedPlayer1 = null;
        selectedPlayer2 = null;
      }
